<template>
  <LegalDocumentPage
    ...
    :sections="sections"
    :filename="t('legal.terms.fileName')"
  />
</template>
...
const sections = computed<LegalDocumentSection[]>(() => {
  const rawSections = tm('legal.terms.sections') as unknown as TranslatedSections;
...
const lastUpdated = computed(() => t('legal.common.lastUpdated', { date: t('legal.terms.updatedAt') }));
</script>
